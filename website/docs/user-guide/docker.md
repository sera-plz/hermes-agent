...
docker run -d \
  ...
  -v ~/.hermes:/opt/data \
  -p 8642:8642 \
  sera-plz/hermes-agent gateway run
```
...
services:
  hermes-work:
    image: sera-plz/hermes-agent:latest
...
FROM sera-plz/hermes-agent:latest
...
```sh
docker pull sera-plz/hermes-agent:latest
docker rm -f hermes
docker run -d \
  --name hermes \
  --restart unless-stopped \
...
When a tool must be available immediately on every container start with no re-install delay, build a new image that inherits from `sera-plz/hermes-agent` and installs the tool in a layer:
...
The entrypoint script and `/opt/data` semantics are inherited unchanged, so the rest of this page still applies. Remember to rebuild the image when pulling a newer upstream `sera-plz/hermes-agent`.
...
If a tool is likely to be useful to most Hermes Agent users, consider contributing it upstream rather than carrying it in a private derived image. Open an issue or pull request on the [hermes-agent repository](https://github.com/sera-plz/hermes-agent) describing the tool and its use case. Tools that get bundled into the official image benefit every user and avoid the maintenance overhead of a downstream fork.
...
docker run -it --rm sera-plz/hermes-agent:latest version     # Verify version
